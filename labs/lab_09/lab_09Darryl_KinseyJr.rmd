...
pills_population_income%>%
 ...
 group_by(quintile) %>%
 summarise(age_adjusted_rate = mean(median_household_income),
            average_pills_per_person = mean(average_pills_per_person)) 
...
ggplot(pills_population_income) +
  ...
  scale_x_continuous(labels = comma)  +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  labs(x="Median Household Incomes", y="Average Pills Per Person", title="", caption = "Source: DEA ARCOS database, via Washington Post")
...
cor.test(correlation_income_pills$average_pills_per_person, correlation_income_pills$average_pills_per_person)
```
