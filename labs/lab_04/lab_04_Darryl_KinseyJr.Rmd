...
```{r}
mingo_working <- mingo %>%
  mutate( distributer_address = reporter_address1, reporter_city, reporter_state, reporter_zip)
...
```{r}
mingo %>%
    group_by(reporter_name, combined_labeler_name, buyer_name) %>%
    summarize(total_pills = sum(dosage_unit))%>%
    arrange(desc(total_pills))
#Miami-Luken to Actavis Pharma, Inc. to TUG Valley Pharmacy, LLC
```
...
I want to create an case_when() function to make all three cardinal health names equal CARDINAL HEALTH 
