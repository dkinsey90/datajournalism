...
```{r}
Estimate_Total_White_alone <-get_acs(geography = "county", 
              variables="B01002A_003", year=2014,  geometry = FALSE )
```
```
...
```{r}
  ...
  #for some reason this part stopped showing up as code, I don't know if it's something with r studio or something i didn't install. i ran all five packages but this (the web scraping) is the only part acting as plain text. additionally, it wouldn't allow me to knit with this typed in.
