...
# Transaction ID's, I don't really see a need to do math in that column. I think all of the character listed columns are fine. 
#missing values = null
```
...
#colnames(baltimore)
baltimore %>%
  group_by(buyer_name, buyer_address1, buyer_city, buyer_state, buyer_zip) %>%
  summarize(total_pills = sum(dosage_unit)) %>%
  arrange(desc(total_pills))
#NEWCARE HOME HEALTH SERVICES
#The state of Maryland yanked their license, and the DEA busted them for selling over 10 million pills illegally and $20 million in money laundering
#"Newcare Home Health Services located at 3423 Sinclair Lane, received over 61 million pills between 2006 and 2012. In 2006, the State of Maryland suspended their license to practice over fraud allegations, before the pharmacy's owners: 51-year-old Steven Abiodun Sodipo and 49-year-old Callixtus Onigbo Nwaehiri, were convcited of fraud, money laundering, and failure to pay taxes."
```
...
```{r}
baltimore %>%
  group_by(reporter_name, reporter_address1, reporter_city, reporter_state, reporter_zip) %>%
  summarize(total_pills = sum(dosage_unit)) %>%
  arrange(desc(total_pills))
#DBA Mckesson Drug Company, sendig 49.834,830 plls from 2006-2012. Their corporate office is located in Irving, TX
```
...
#Rite Aid of Maryland Inc, located on 5804 Ritchie Highway. The labler name was under two different names that shipped to that pharmacy.
```
...
baltimore %>%
    group_by(buyer_name) %>%
    summarize(total_pills = sum(dosage_unit )) %>%
    mutate(total_pills= percent_rank(total_pills)) %>%
    arrange(desc(total_pills))
#RITE AID OF MARYLAND,INC, 100%
