...
```{r}
Maryland <-arcos_county_pills_per_year%>%
...
```{r}
ggplot(Maryland) +
geom_bar(stat='identity', aes(year, dosage_unit), fill="blue")+
  labs(x="Year", y="Total pills", title="Spread of opioids through Maryland") +
...
```{r}
#The state had a gradual rise into a peak in 2011, compared to Baltimore City that had an explosion of pills in 06, followed by a crash and gradual rebuild
```
...
```{r}
#pillcount
state_pills<- arcos_county_pills_per_year%>%
  filter(year=="2012")%>%
...
```{r}
#join
America_pills <- state_pills%>%
  inner_join(State_population, by="buyer_state")%>%
  select(dosage_unit, population, buyer_state)
```
