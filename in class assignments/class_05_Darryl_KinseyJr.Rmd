...
```{r}
#buyer_address <- read.csv("data/buyer_addresses.csv")
#pill_totals <- read.csv("data/buyer_totals.csv")
#buyer_year <- read.csv("data/buyer_annual_by_year.csv")
#state_pops <- read.csv("data/state_population_per_year.csv")

#buyer_totals <- pill_totals
rm(pill_totals)
...
```{r}
#Buyer year has an extra variable compared to toals. address has several different values compared to totals.
```
...
```{r}
#pill_buyers <-buyer_address %>%
 # inner_join(buyer_totals, by="buyer_dea_no")%>%
  #arrange(desc(total_pills))
 
#VA Consolidated Outpatient Pharmacy, North Charleston, SC. This Pharmacy did a deal with McKesson for deliveries of pills.
```
...
```{r}
#pills_by_year <-pill_buyers%>%
  #right_join(buyer_year)%>%
  #arrange(year, total_pills, buyer_name, buyer_state.x)

#Rakesh K Mathur, MD, 579,110 pills. #license to practice was suspended in 2012, assessed a $25 thousand dollar fine, and put on probabtion for five years. Mathur's suspension was terminated upon payment of the fine in Feb. 2015, though his probation, which ends this year, remains. 
